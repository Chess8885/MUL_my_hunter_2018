...
void duck_tick(t_game *game)
{
    if (game->duck.seconds >= 0.12) {
        game->duck.rect.left = game->duck.rect.left + 110;
        if (game->duck.rect.left >= 330)
    ...
    game->duck.pos.x = game->duck.pos.x + 0.09f;

    if (game->duck.pos.x > 800) {
        game->duck.pos.x = -120;
        game->duck.pos.y = rand() % 400;
    }
}
